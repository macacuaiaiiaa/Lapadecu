...
const TVShows: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTVShows = async () => {
      try {
        ...
        if (yearParam) {
          params.first_air_date_year = yearParam;
        }
  ...
  const getActiveGenreName = () => {
    if (!genreParam) return 'Todas as Séries';
  ...
  const sortOptions = [
    ...
    { value: 'first_air_date.desc', label: 'Lançamento (Recente)' },
    { value: 'first_air_date.asc', label: 'Lançamento (Antigo)' }
  ];
